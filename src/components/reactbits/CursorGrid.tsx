...
// React Bits — CursorGrid.
// 커서 주변 셀이 빛나는 canvas 그리드 (radius / falloff / click pulse 설정 가능).
//
// 수정점: fixed 배경 레이어로 쓰면 container 에 pointer-events:none 이 걸려
// container 가 포인터 이벤트를 못 받는다. 그래서 pointermove/pointerdown 을
// container 가 아니라 **window** 에서 받도록 변경했다. (좌표는 canvas rect 기준)
...
const CursorGrid = ({
  ...
  useEffect(() => {
    ...
    // canvas rect 기준으로 로컬 좌표 계산 (canvas 는 fixed 전체화면이라 rect.left/top≈0)
    ...
    // fixed 배경(pointer-events:none)이라 container 대신 window 에서 포인터 이벤트 수신
